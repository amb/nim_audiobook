import sugar, sequtils, math, fft, random
...
# proc basicSine*(srate: int): seq[float] =
#     let srate = float(srate)
#     let dur = 1000
#     let rr_dur = dur * 2
#     let rr = toSeq(0..<rr_dur).mapIt(it.float).toseq()
...
proc stft*(data: seq[float], fft_size: int): seq[Complex[float]] =
...
proc griffin_lim*(mag_spec: seq[float], iterations: int): seq[float] =
    ...
    ds.wave = randomseq(samples_size, 1.0)
